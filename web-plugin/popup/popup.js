...
const safeChecksEl = document.getElementById("safe-checks");
const safeReanalyzeEl = document.getElementById("safe-reanalyze");
const certNoteEl = document.getElementById("cert-note");
...
function setResult(type, mark, title, desc, detail = "", badge = "") {
  ...
  if (safeChecksEl) safeChecksEl.hidden = type !== "safe";
  if (certNoteEl) certNoteEl.hidden = type !== "safe";
}


function setToggle(enabled, riskLevel) {
  currentEnabled = enabled;
  toggleEl.classList.remove('is-on', 'is-safe', 'is-warning', 'is-danger');
  ...
  if (enabled) {
    ...
    if (riskLevel === 'warn') {
      ...
      toggleEl.classList.add('is-safe');
      toggleLabelEl.textContent = '安全';
    } else {
...
async function render(forceAi = false) {
  ...
  if (aiWrapEl) aiWrapEl.hidden = true;
  setToggle(enabled, "safe");
  setResult("safe", "安全", "安全", "当前网站安全可靠，可放心访问。", "", "安全");
}

toggleEl.addEventListener("click", async () => {
  const nextEnabled = !currentEnabled;
  setToggle(nextEnabled, "checking");
  setUserToggled(true);
...
safeReanalyzeEl?.addEventListener("click", async () => {
  await render(true);
});
