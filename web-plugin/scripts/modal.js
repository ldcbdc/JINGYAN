const JINGYAN_MODAL = (() => {
  ...
  function removeModal() {
    if (active) closeActive("dismissed");
    else document.getElementById(MODAL_ID)?.remove();
  }

  function showModalDialog({ level, title, message, detail, showCancel, dedupKey, timeout, okText, cancelText }) {
    return new Promise(resolve => {
      ...
      titlebarIcon.className = 'titlebar-icon';
      ...
      titlebar.append(titlebarIcon, titlebarText);
      ...
      if (timeout) {
        ...
        const countdownNumEl = document.createElement('span');
        ...
        timerId = setInterval(() => {
          seconds--;
          countdownNumEl.textContent = String(Math.max(0, seconds));
          if (seconds <= 0) closeActive('timeout');
        }, 1000);
      }
      ...
      if (showCancel) {
        ...
        cb.addEventListener('click', () => closeActive('cancel'));
        footer.appendChild(cb);
      }

      const okBtn = document.createElement('button');
      okBtn.className = 'btn btn-primary';
      okBtn.textContent = okText || (showCancel ? '确定' : (isDanger ? '终止访问' : '知道了'));
      okBtn.addEventListener('click', () => closeActive('ok'));
  ...
  return { MODAL_ID, removeModal, showModalDialog, clearAlertKey };
})();
